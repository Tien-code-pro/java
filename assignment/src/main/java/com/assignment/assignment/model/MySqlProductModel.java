...
public class MySqlProductModel implements ProductModel{
    ...
    public Product save(Product product) {
        try {
            ...
            String sqlQuery = "insert into products "+
                    "(id,name,image,price,qty,color_id,content,category_id,size_id,rate,mass,createdAt,updatedAt,status)"+
                    "value "+"(?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
            ...
            preparedStatement.setInt(9,product.getSize_id());
            preparedStatement.setInt(10,product.getRate());
            preparedStatement.setString(11,product.getMass());
            preparedStatement.setString(12,product.getCreatedAt().toString());
            preparedStatement.setString(13,product.getUpdatedAt().toString());
    ...
    public List<Product> findAll() {
        ...
        try {
            ...
            while (resultSet.next()){
                ...
                int size_id = Integer.parseInt(resultSet.getString("size_id"));
                ...
                Product product = new Product(id,name,image,price,qty,color_id,content,category_id,size_id,rate,mass);
    ...
    public Product findById(String id) {
        ...
        try {
            ...
            while (resultSet.next()){
                ...
                LocalDateTime updatedAt = LocalDateTime.ofInstant(resultSet.getTimestamp("updatedAt").toInstant(), ZoneId.systemDefault());
                int intstatus = resultSet.getInt("status");
                product = new Product(id,name,image,price,qty,color_id,content,category_id,size_id,rate,mass);
                product.setCreatedAt(createdAt);
                product.setUpdatedAt(updatedAt);
                product.setStatus(ProductStatus.of(intstatus));
            }
    ...
    public Product update(String id, Product product) {
        try {
            ...
            String sqlQuery = "update products "+
                    "set id = ?,name = ?,image = ?,price = ?,qty = ?,color_id = ?,content = ?,category_id = ?,size_id = ?,rate = ?,mass = ?, createdAt = ?,updatedAt = ? ,status = ? where id =?";
            PreparedStatement preparedStatement = connection.prepareStatement(sqlQuery);
            preparedStatement.setString(1,product.getId());
            preparedStatement.setString(2,product.getName());
            preparedStatement.setString(3,product.getImage());
    ...
    public List<Product> findBycategoryId(String cid) {
        ...
        try {
            Connection connection = ConnectionHelper.getConnection();
            String sqlQuery = "select * from products where category_id = ?";
            PreparedStatement preparedStatement = connection.prepareStatement(sqlQuery);
